...
public class AddHouseActivity extends AppCompatActivity {
    Button btPostHouse;
    EditText edHouseName, edHouseAddress, edHousePrice, edWaterPrice, edElectricPrice, edHousePhone;
    ...
    private void addEvents() {
        btPostHouse.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String phone = edHousePhone.getText().toString();
                String status = "Còn phòng";
                double latitude = MainActivity.currentLatLng.latitude;
                double longtitude = MainActivity.currentLatLng.longitude;
                boolean verified = false;

                final House house = new House(address, electricPrice, waterPrice, housePrice, longtitude, phone, name, status, verified, latitude);
                DatabaseReference mRef = FirebaseDatabase.getInstance().getReference().child("NhaTro");
                mRef.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        count=dataSnapshot.getChildrenCount();
                        Toast.makeText(getApplicationContext(),count+"",Toast.LENGTH_SHORT).show();
                        DatabaseReference mRef1 = FirebaseDatabase.getInstance().getReference().child("NhaTro/"+count);
                        mRef1.setValue(house);
    ...
    private void addControls() {
        ...
        edHousePhone = (EditText) findViewById(R.id.edHousePhone);
    }
